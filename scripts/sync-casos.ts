...
async function main() {
  const { count, soql, geocodificados, ubicados } = await syncCasos()
  console.log('SOQL:', soql.replace(/\s+/g, ' ').trim())
  console.log(`Sync completo: ${count} casos · ${ubicados} ubicados en el mapa · ${geocodificados} geocodificadas.`)
}
